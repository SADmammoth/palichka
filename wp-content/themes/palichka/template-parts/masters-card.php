...
<article id="post-<?php the_ID(); ?>" class='vflex-item masters-card card'>
  <a href='<?php echo get_permalink(); ?>' class='photo-box'>
    <img class='photo-thumb' width='150 ' height='150 '  src='<?php
...
?></span></label>
  ...
  <?php else: ?>
    <p class='description hyphenated'><?php
    $str = rwmb_meta('masters-desc');
    echo strlen($str) > 350 ? mb_substr($str, 0, 350) . '...' : $str;
    ?></p>
  <?php endif;
  ?>
  </div>
