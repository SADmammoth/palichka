...
$args = array(
  ...
  <div>  
    ...
    <?php 
     ...
     ?>
        <option <?php echo (  explode(',',$topic)[0] == 'false' && explode(',',$topic)[1] == 'false')?'selected':''; ?> value='<?php echo 'false,false'?>'><?php echo 'Все разделы, Все теги'?></option><?php
      foreach ($categories as $cat){
        $tags = get_terms(['taxonomy'=>$cat->name.'_tag', 'hide_empty'=>false]);
        ?>   <option <?php echo (  explode(',',$topic)[0] == $cat->name && explode(',',$topic)[1] == 'false')?'selected':''; ?> value='<?php echo $cat->name.',false'?>'><?php echo $cat->label.', Все теги'?></option><?php
        echo(print_r($tags));
        foreach ($tags as $one_tag){
          ?>
          <option <?php echo (  explode(',',$topic)[0] == $cat->name && explode(',',$topic)[1] == $one_tag->slug)?'selected':''; ?> value='<?php echo $cat->name.','.$one_tag->slug?>'><?php echo $cat->label.', тег: '.$one_tag->name?></option><?php
        }
      }
    ?>
    </select>
