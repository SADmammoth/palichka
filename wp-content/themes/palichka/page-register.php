...
get_header(); ?>
	<div id="primary" class="content-area">
     ...
     <section id ='main-section' style='min-height: 20vw; height: auto; padding-bottom: 50px;'>
      ...
      <h2 style='margin-bottom: -5px; padding-top: 20px;'><?php echo isset($_GET["signin"])
        ? "Вход"
        : "Регистрация"; ?></h2>
      <fieldset class='form-head' style='position: relative;'>
        <input class='form-input' id='name' name='username' type='text' placeholder='Логин<?php echo isset(
          $_GET["signin"]
        )
          ? " или Email"
          : ""; ?>' onchange='<?php echo isset($_GET["signin"])
  ? "check_signin(this)"
  : "check_validity(this)"; ?>' required/>
        <input class='form-input<?php echo isset($_GET["signin"])
          ? " hidden"
          : ""; ?>' name='email' type='email' placeholder='Email'  onchange='check_validity(this)' require/>
        ...
        <input class='form-input<?php echo isset($_GET["signin"])
          ? " hidden"
          : ""; ?>' id='repeat_password' type='password' placeholder='Повторите пароль' require/>
        <div id='password_error' class='hint error-message hidden' style='position: absolute; left: calc(100% + 10px); width: 80%; top: 10px;'>
        <i class='fas fa-times-circle' style='float: left; padding-right: 15px; clear: none;'></i>
        <span class='message' style='float: left; clear: none; width: 90%;'></span>
        </div>
        </fieldset>
        <input type='hidden' name='reg' value='<?php echo isset($_GET["signin"])
          ? 'false'
          : 'true'; ?>'/>
        <input type='hidden' name='redirectpath' value='./redirect/?sigin=true'>
        <a class='link' href='<?php echo get_site_url() .
          '/register/' .
          (isset($_GET["signin"]) ? '' : '?signin="true"'); ?>' ?><?php echo isset($_GET["signin"])
  ? "Регистрация"
  : "Вход"; ?></a>
        <button type='submit' class='button' style='float: right'><?php echo isset($_GET["signin"])
          ? "Войти"
...
if (is_user_logged_in()) { ?>
  <script>
      if(confirm('Вы уже вошли на сайт "Свая Палiчка", желаете выйти?')){
      <?php wp_logout(); ?>
       document.location.href= "<?php echo get_site_url() . '/register'; ?>";
    }else{
      document.location.href= "<?php echo get_site_url() . '/404'; ?>";
    };
  </script>
  <?php }
