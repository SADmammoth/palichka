  ...
  function kirki_init()
  {
    ...
    function widget_choice($type)
    {
      ...
      foreach ($widget_ids as $widget_id) {
        if (is_int($widget_id)) {
          $result[$type . '-' . $widget_id] = reset(
            get_widget($type . '-' . $widget_id, ['title'])
          );
        }
      }
      return $result;
    ...
    Kirki::add_field('mainpage_nav', [
      ...
      'description' => esc_html__(
        '(не имеет эффекта, если флаг "Отображать панель в главной секции" отключен)',
        'palichka'
      ),
      'section' => 'main_block',
      'priority' => 11,
    ...
    Kirki::add_field('topic_widgets', [
      ...
      'fields' => [
        ...
        'topic' => [
          ...
          'default' => 'false,false',
          'choices' => array_merge(
            ['false,false' => 'Все разделы, все теги'],
            posttypes_tags_choice('_tag')
          )
        ],
        // 'topic'  => [
        //   'type'        => 'select',
