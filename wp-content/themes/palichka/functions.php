...
if (!function_exists('palichka_setup')):
  ...
  function palichka_setup()
  {
    ...
    add_theme_support(
      'custom-background',
      apply_filters('palichka_custom_background_args', array(
...
function palichka_scripts()
{
  ...
  wp_enqueue_style(
    'palichka-customize-root-vars',
    get_template_directory_uri() . '/layouts/customize-root-vars.php'
  );
  wp_enqueue_style(
    'fontawesome-style',
    get_template_directory_uri() . './layouts/fontawesome-free-5.1.1-web/index.css'
  );
  wp_enqueue_style('palichka-style', get_stylesheet_uri());

  wp_register_script(
    'palichka-registration',
  ...
  wp_register_script(
    'palichka-like',
    get_template_directory_uri() . '/js/like.js',
    array(),
    1.0,
    true
  );
  wp_register_script(
    'palichka-hyphenate',
    get_template_directory_uri() . '/js/hyphenate.js',
    array(),
    '2.0',
    true
  );
  ...
  wp_enqueue_script(
    'palichka-scripts',
    get_template_directory_uri() . '/js/minor-scripts.js',
    array(),
    null,
    true
  );
  wp_enqueue_script(
    'palichka-skip-link-focus-fix',
...
{
  wp_enqueue_script('my-block-editor-js', get_template_directory_uri() . '/js/blocks.js', [
    'wp-blocks',
    'wp-element',
    'wp-components',
    'wp-i18n'
...
function ts_hide_pages_in_wp_admin($query)
{
  ...
  if (is_admin() && $pagenow == 'edit.php' && $post_type == 'page') {
    $query->query_vars['post__not_in'] = array('655', '663');
  }
}
...
function get_current_user_avatar()
{
  ...
  return wp_get_attachment_image_src(
    get_user_meta(get_current_user_id(), $wpdb->get_blog_prefix() . 'user_avatar', true)
  )[0];
}
