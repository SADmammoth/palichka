...
function retrieve_password()
{
    ...
    if (empty($_POST['user_login']) || !is_string($_POST['user_login'])) {
        $errors->add(
            'empty_username',
            __('<strong>ERROR</strong>: Enter a username or email address.')
        );
...
switch ($action) {
	...
	<p>
      ...
      $user_login
  ); ?>" size="20" autocapitalize="off" /></label>
	</p>
		 /**
    ...
    default:
        ...
        if (!is_wp_error($user) && !$reauth) {
            if ($interim_login) {
                ...
                ?>
				 ...
				 /** This action is documented in wp-login.php */<?php
        ...
        if ($interim_login) {
            ...
            if (isset($_GET['loggedout']) && true == $_GET['loggedout']) {
                ...
                $errors->add(
                    'confirm',
                    __('Check your email for the confirmation link.'),
                    'message'
                );
            } elseif (
                isset($_GET['checkemail']) &&
                ...
                $errors->add(
                    'newpass',
                    __('Check your email for your new password.'),
                    'message'
                );
            } elseif (
                isset($_GET['checkemail']) &&
